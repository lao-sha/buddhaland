...
pub mod pallet {
    ...
    pub enum Event<T: Config> {
        ...
        Exchanged(T::AccountId, u128, KarmaBalance, u128, u128, u128),
    }
    ...
    impl<T: Config> Pallet<T> {
        /// 用户用BUD兑换Karma，并将BUD按比例分配到黑洞/国库/paymaster
        /// - amount: 兑换的BUD数量
        /// 安全性：
        /// - 检查比例之和==BpsDenominator
        ...
        pub fn exchange(origin: OriginFor<T>, amount: u128) -> DispatchResult {
            ...
            let bps_sum = T::BurnBps::get().saturating_add(T::TreasuryBps::get()).saturating_add(T::PaymasterBps::get());
            ...
            Self::deposit_event(Event::Exchanged(who, amount, karma_out, burn, treasury_total, paymaster));
            Ok(())
        }
    }
}