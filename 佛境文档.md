...
### 免签名代付（增强）
- 机制：预授权（额度/有效期/上限）+ 系统托管池（提前注资）+ 交易扩展（校验/准备）
- 流程：授权赞助方 -> 注资系统池 -> 用户发起 request_auto_pay -> 区块 on_initialize 批处理
- 风险控制：白名单、每块限速、事件可观测、失败重试
- 与 Exchange/Karma 的配合：可基于 Karma 等级授予月度免签额度（产品策略层）
