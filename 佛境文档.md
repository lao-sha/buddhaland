...
- **模块化集成**：提供标准接口供其他 pallet 向托管池注入资金
- **无直接依赖**：通过标准接口与其他模块交互，避免循环依赖
- **系统托管池设计**：
- **资金来源多样化**：支持来自 exchange pallet、share-mining pallet 等多种资金来源
- **无直接依赖**：通过标准接口与其他模块交互，避免循环依赖
- **集中管理**：统一管理所有外部注入的资金，确保资金安全和使用效率

**与其他模块的交互**：
- **Exchange Pallet**：接收 BUD 兑换产生的手续费分配
- **Share-Mining Pallet**：可接收分享挖矿产生的手续费
- **其他未来模块**：通过 `increase_system_pool()` 标准接口接收资金注入
